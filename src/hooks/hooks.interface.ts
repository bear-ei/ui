...
export interface UseHandleStateEventOptions
    extends Partial<PressableProps & OnStateEvent> {
    disabled?: boolean
    onStateEventChange?: (
        options: OnStateEventChangeOptions
    ) => (state: State) => (event: StateEvent) => void
}
...
export type UseDesktopScrollEventOptions = Pick<
    ScrollViewProps,
...
export interface AnimatedTimingOptions
    extends Omit<WithTimingConfig, 'duration' | 'easing'> {
    duration?: Duration
    easing?: Easing
    repeat?: number
...
export interface HandleAnimatedTimingOptions
    extends Omit<AnimatedTimingOptions, 'sharedValue' | 'callback'> {
    token: Token
}
