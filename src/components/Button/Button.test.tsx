...
describe('Button', () => {
	...
	it('should render as LINK type with active indicator when eventName is hoverIn', () => {
		...
		const button = getByTestId('btn-link')
	...
	it('should not crash when disabled is false and type is FILLED', () => {
		...
		expect(getByTestId('btn-4')).toBeTruthy()
	})
	...
	it('should support error state and render correct color', () => {
		...
		const animatedText = getByTestId('button__animatedLabelText--test-id')
	...
	it('should trigger onPressOut callback when pressOut event occurs', async () => {
		...
		const touchable = getByTestId('touchable__touchableContent--test-id')
