...
const handleTouchableContentLayout =
    (setState: Updater<TouchableState>) => (event: LayoutChangeEvent) => {
    ...
    (touchableLocation?: Pick<NativeTouchEvent, 'locationX' | 'locationY'>) =>
        setState(draft => {
            draft.rippleSequence[
                `${Date.now()}${generateRandomNumberString(4)}`
            ] = {touchableLocation}
        })
...
const handleTouchableStateChange =
    ...
    (event: StateEvent) => {
        const nextEvent = {
            layout: () =>
                handleTouchableContentLayout(setState)(
                    event as LayoutChangeEvent
                ),
            pressIn: () =>
                handleTouchablePressIn(setState)(enableTouchableRipple)(
...
const handleTouchableAnimatedFinished =
    (setState: Updater<TouchableState>) => (index: string) =>
        setState(draft => {
...
const renderTouchableRipples =
    ...
    (rippleSequence: TouchableRippleSequence) =>
        containerLayout?.width ?
            Object.entries(rippleSequence).map(
                ([index, {touchableLocation}]) => {
...
export const TouchableBase = forwardRef<View, TouchableBaseProps>(
    (
        ...
        const [{rippleSequence, contentLayout}, setState] =
            useImmer<TouchableState>({
        ...
        const onTouchableAnimatedFinished = useMemo(
            () => handleTouchableAnimatedFinished(setState),
            [setState]
        )
