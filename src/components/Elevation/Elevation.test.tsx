...
describe('Elevation', () => {
	it('should render container and shadow with correct testID', () => {
		...
		expect(getByTestId('test-shadow')).toBeTruthy()
		expect(getByTestId('elevation__animatedShadow--test-id')).toBeTruthy()
	})
	...
	it('should use `level` over `defaultLevel` when both are provided', () => {
		...
		const shadow = getByTestId('elevation__animatedShadow--test-id')
