...
const handleMenuKeyDown = ({
    ...
    const handleMenuKeyCode =
        (setState: Updater<MenuState>) => (keyCode?: string) =>
            setState(draft => {
                ...
                switch (true) {
                    ...
                    case keyCode?.startsWith('ArrowDown'):
                        draft.focusedIndex =
                            currentFocusedIndex + 1 > lastIndex ?
                                0
                            :   currentFocusedIndex + 1
                        break
                    ...
                    case keyCode?.startsWith('Enter') &&
                        ...
                        if (
                            !focusData ||
                            typeof draft.focusedIndex !== 'number'
                        ) {
                            return
                        }

                        if (multiple) {
                            draft.nextActivesEvent = handleNextActivesEvent(
                                focusData.indexKey
                            )
                        } else {
...
const handleMenuVisible =
    ...
    (onVisible?: (value?: boolean) => void) => {
        const handleNextVisibleEvent = (value?: boolean) => () =>
            onVisible?.(value)
...
>(
    (
        ...
        const [
            {focusedIndex, nextVisibleEvent, nextActivesEvent, nextActiveEvent},
            setState
        ] = useImmer<MenuState>({
        ...
        const onMenuVisible = useMemo(
            () => handleMenuVisible(setState)(onVisible),
            [onVisible, setState]
        )
