...
const handleFABElevation =
    (draft: WritableDraft<FABState>) =>
    (elevated?: boolean) =>
    (state?: State) => {
        if (!elevated) {
            return
    ...
    (theme: DefaultTheme) => {
        ...
        return (icon?: JSX.Element) => {
            ...
            const iconSize = theme.adaptSize(
                theme.token.spacing.large + 3 * theme.token.spacing.extraSmall
            )
...
export const FABBase = forwardRef<View, FABBaseProps>(
    (
        ...
        const onFABDisabled = useMemo(
            () => handleFABDisabled(setState)(elevated),
            [elevated, setState]
        )

        const onFABInit = useMemo(
            () => handleFABInit(setState)(disabled),
            [disabled, setState]
        )

        const fabIconElement = renderFABIcon({eventName, type, disabled, size})(
            theme
        )(icon)
