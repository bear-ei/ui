...
export const Shape = styled.View<ShapeProps>`
    ${({shape = 'none', theme}) => css`
        border-bottom-left-radius: ${theme.adaptSize(
            theme.token.shape[shape].bottomLeft
        )}px;

        border-bottom-right-radius: ${theme.adaptSize(
            theme.token.shape[shape].bottomRight
        )}px;

        border-top-left-radius: ${theme.adaptSize(
            theme.token.shape[shape].topLeft
        )}px;

        border-top-right-radius: ${theme.adaptSize(
            theme.token.shape[shape].topRight
        )}px;
    `}
`
...
export const Typography = styled.Text<TypographyProps>`
    ${({theme, type = 'title', size = 'medium'}) => css`
        ...
        font-size: ${theme.adaptFontSize(
            theme.token.typography[type][size].size
        )}px;
        ...
        height: ${theme.adaptSize(
            theme.token.typography[type][size].lineHeight
        )}px;

        letter-spacing: ${theme.adaptSize(
            theme.token.typography[type][size].letterSpacing
        )}px;
